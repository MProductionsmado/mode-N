...
class DiscreteDiscreteDiffusionModel3D(nn.Module):
    ...
    def p_sample(
        ...
        Args:
            x: Current noisy state (B, C, D, H, W)
            t: Timestep (B,)
            text_embed: Text embeddings (B, text_dim)
            text_proj: Projected text embeddings (B, proj_dim)
            size: Size category
        ...
        # Estimate x_0 distribution from logits
        ...
        if t[0] > 0:
            ...
            # Correct posterior calculation for multinomial diffusion
            # q(x_{t-1} | x_t, x_0) using Bayes rule
            
            # Transition probabilities from x_0 to x_{t-1}
            ...
            # Transition probabilities from x_0 to x_t  
            ...
            # For multinomial: this becomes element-wise product and renormalization
